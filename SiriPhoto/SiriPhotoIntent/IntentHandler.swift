...
//    }
    ...
    func resolvePeopleInPhoto(forSearchForPhotos intent: INSearchForPhotosIntent, with completion: @escaping ([INPersonResolutionResult]) -> Void) {
        completion([INPersonResolutionResult.notRequired()])
    }
    
    func resolveLocationCreated(forSearchForPhotos intent: INSearchForPhotosIntent, with completion: @escaping (INPlacemarkResolutionResult) -> Void) {
...
//    
    ...
    func handle(searchForPhotos intent: INSearchForPhotosIntent, completion: @escaping (INSearchForPhotosIntentResponse) -> Void) {
        let response = INSearchForPhotosIntentResponse(code: INSearchForPhotosIntentResponseCode.continueInApp, userActivity: nil)
        
        if intent.dateCreated != nil{
            let calender = Calendar(identifier: .gregorian)
            let startDate = calender.date(from: (intent.dateCreated?.startDateComponents)!)
            let endDate = calender.date(from: (intent.dateCreated?.endDateComponents)!)
            
            response.searchResultsCount = photoSearchFrom(startDate!, to: endDate!)
        }
